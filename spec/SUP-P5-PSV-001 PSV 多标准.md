...
版本：V1.5.3
日期：2026-09-16
状态：**已批准**（V1.5.1 项目评审通过；V1.5.2 关闭 P5-OPEN-00R D-08 — 工艺室 2026-09-16 核查结论回填 §4.4 / §2.1 / §5.5；V1.5.3 新增 §10.1 P5+ 评估参考公式库 + P5-OPEN-00S）
...
修订说明
---
V1.5.3 相对 V1.5.2 新增 §10.1「P5+ 评估参考公式库」+ P5-OPEN-00S：

- 用户 2026-09-16 提供 HG/T 20570.2-1995 未提供完整公式的两个工况（控制阀故障 + 换热器管破裂）的公开文献补充方案（曹虎等《化工技术与开发》2014(08) + HG20570 海川化工论坛讨论 + 《管壳式换热器中管子断裂泄放量的计算》等）
- **强制声明（§10.1 顶部）**：公式来源为公开文献经验总结，非 HG/T 20570.2-1995 标准原文条款；仅用于 P5+ 评估初步选型参考，不可直接进入 P5 计算链
- §10.1.1 控制阀故障（液相）：阻塞流判别 + F_F + C_v + W_max 推导链 + 等百分比行程系数 1.3~1.5（**P5+ 实施时工艺室按项目给定；保守参考默认值取 1.5**）
- §10.1.2 换热器管破裂（液相高压侧）：W = 5.6 d² √(G·Δp) + 双面争议说明 + **C 值 0.74（HG20570）/ 0.6（CRANE）双取值并列，P5+ 评估时由工艺室选定**
- §10.1.3 换热器管破裂（气相/两相流）：HG20570 无 + 国内外文献无共识公式；扩展新 OPEN 项 **P5-OPEN-00S**，逐案分析，工艺室 + 标准负责人联签
- §8.1 + §9 OPEN 项表同步新增 00S 行（V1.5.3 新增标识）
- 不动 §4.3 / §4.4 P5 实施边界；G8 / G8c 门禁 + §7 验收表 / §8.5 测试基线均无变化
...
| **P5-OPEN-00S**（V1.5.3 新增） | 待 P5+ 评估 | 换热器管破裂气相/两相流工况：HG/T 20570.2-1995 无公式 + 国内外文献无共识公式（详 §10.1.3）；逐案分析，工艺室 + 标准负责人联签 |
...
| **P5-OPEN-00T**（M1 V1.5.1 新增） | GB/T 150.1 附录B 火灾分支范围变更（从 P5 交付范围移至前置条件） | Task 13 GB 分支前置 | P5 启动评审会确认（工艺室签字对账表） |
| **P5-OPEN-00S**（V1.5.3 新增） | 换热器管破裂气相/两相流工况（HG/T 20570.2 + 国内外文献均无共识公式） | Task 14 GB 分支 / Task 16 两相流 / G8c | 工艺室 + 标准负责人联签，P5+ 评估逐案分析 |
...
### §10.1 P5+ 评估参考公式库（仅供评审材料，非 P5 实施依据）

> **强制声明**：本节公式来源为公开文献经验总结（曹虎等《化工技术与开发》2014(08) + HG20570 海川化工论坛讨论 + 《管壳式换热器中管子断裂泄放量的计算》等），**非 HG/T 20570.2-1995 标准原文条款**。
>
> **用途限定**：仅用于 P5+ 评估初步选型参考，**不可直接进入 P5 计算链**。P5 阶段控制阀故障 + 换热器管破裂工况一律触发 G8 / G8c 拒绝（详 §4.3 + §7 验收表），resolver 层抛 `PsvProfileInsufficientError`，无计算函数调用。

#### §10.1.1 控制阀故障工况（closed_valve）液相泄放量

**HG/T 20570.2-1995 标准原文缺口**：仅规定 `安全阀泄放量 = 控制阀最大通过量 − 正常流量`，但未给出"控制阀最大通过量"的计算公式。

**文献补充公式**（曹虎等，《化工技术与开发》2014(08)）：

```
W_relief = W_max − W_normal
```

`W_max`（阀门全开时控制阀液体流量）推导步骤：
...
- `p1`：阀门入口处压力，kPa
- `p_v`：液体饱和蒸汽压，kPa
- `F_L`：压力恢复系数（多数控制阀 0.8~0.9，工程计算可假定 **0.85**）
- `F_F`：液体临界压力比系数
...
若控制阀尚未采购，无法由供货商提供全开时的 C_v，按以下经验估算：

- 假定阀门为**等百分比特性**
- 正常操作时阀门开度宜在 30%~75%，取稳态最大流量对应行程 75%
- 故障全开时行程 100%
- 等百分比特性下：`C_v(x) = a × e^(b·x)` 或按经验比例估算：**全开 C_v2 约为 75% 行程 C_v1 的 1.3~1.5 倍**（取决于具体阀特性）
- **P5+ 实施时工艺室按项目给定；保守参考默认值取 1.5**

**步骤 5：反算 W_max**

用步骤 4 得到的 `C_v2` 代入步骤 3 公式反算 `Q_L,max`，再乘以密度得到 `W_max`。

**关键假设与局限**（文献自述）：

> "工程实践中，遇到此种情况，常以正常流量乘以一定倍数的安全裕量作为控制阀的最大通过量。这种经验做法，缺乏理论分析支持，会带来安全隐患。"

上述补充公式的定位是"项目初期安全阀选型给予更为准确的数据支持"，**不适用于最终设计定案**。

#### §10.1.2 换热器管破裂工况（heat_exchanger_tube_rupture）液相高压侧

**HG/T 20570.2-1995 标准原文缺口**：未提供换热器管破裂工况完整公式。

**液相高压侧补充公式**（海川化工论坛引用 HG20570 出现公式）：
...
- `W`：泄放量，kg/h
- `d`：破裂管的内径，mm（**争议**：有文献指出应按两个面计算，横截面积 = 2 × π × d² / 4，系数可能需相应调整）
- `G`：液相相对密度
- `Δp`：高压侧与低压侧的压差，bar
- 适用条件：高压流体为液相

**C 值取值争议**（管程→壳程公式）：论坛讨论指出 C 值默认 **0.74**（HG20570），而非 CRANE 手册中的 **0.6**。P5+ 评估时由工艺室选定（不是 P5 内部裁决，是 P5+ 评审材料决策点）；SPEC 此处**两种取值并列列出**，P5+ 实施时按项目选定。

#### §10.1.3 换热器管破裂工况（heat_exchanger_tube_rupture）气相 / 两相流

**HG/T 20570.2-1995 标准原文缺口**：未提供。

**文献现状**：

> "管壳式换热器在换热管断裂工况下的泄放量计算是工程设计的难点，泄放介质为两相流的工况更为复杂。"

国内外文献（《管壳式换热器中管子断裂泄放量的计算》《换热管断裂工况下两相流泄放量的计算》等）**无共识公式**。

**P5+ 处理方式**（详 P5-OPEN-00S）：

- 不进 SPEC 公式库，**逐案分析**
- 工艺室 + 标准负责人联签
- 若高压侧为气相或两相流，§10.1.2 液相公式**不适用**，需另行评估
