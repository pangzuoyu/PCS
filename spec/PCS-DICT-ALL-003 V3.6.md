PCS-DICT-ALL-003 V3.6（折标煤 + DETAIL/HTRI 模板版）
文件标识	PCS-DICT-ALL-003
当前版本	V3.6
发布日期	2026-09-04
变更来源	V3.5 + P2 Sprint 1.10 Task 1.10.1/1.10.2/1.10.3（V1.4 P2-OPEN-005）+ ADR-0029
...
V3.6	2026-09-04	P2 V1.4 P2-OPEN-005 一次性收敛：+ pcs_toe_conversion_factors 表（6 fuel_type seed）+ htri_template_schemas 表（ACHE 33 列 + SHELL_TUBE 23 列）+ TemplateFile.template_version_seq 字段 + CATEGORY_3 折标煤 seed；ADR-0029 同步
...
第七部分：V3.6 增量（P2 V1.4 P2-OPEN-005 一次性收敛，2026-09-04）

本节来自 P2 Sprint 1.10 Task 1.10.1 / 1.10.2 / 1.10.3（commits 7eca3ea / d71fe10 / 18e87ef），ADR-0029 同步。
新增 2 表（53 → 55），新增 1 列（template_files.template_version_seq）。两张新表均含 TimestampMixin 3 列（created_by / created_at / updated_at）。

7.1 新增表：pcs_toe_conversion_factors（折标煤系数组，CATEGORY_3）

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `id` | Integer PK autoincrement | ✅ | 主键 |
| `fuel_type` | String(30)，index | ✅ | GAS / DIESEL / COAL / STEAM / ELECTRICITY / OTHER |
| `toe_conversion_factor` | Numeric(10,4) | ✅ | 吨油当量折算系数 |
| `standard_coal_factor` | Numeric(10,4) | ✅ | 标煤折算系数 |
| `effective_year` | Integer，index | ✅ | 生效年份 |
| `effective_from` | Date | ✅ | 生效起始日 |
| `effective_to` | Date，nullable | ❌ | 生效截止日（NULL = 无限期） |
| `source` | String(200)，nullable | ❌ | 来源 |
| `version` | String(50)，default 'TOE-V1.0' | ✅ | 版本号 |
| `created_by` | UUID，nullable | ❌ | TimestampMixin |
| `created_at` | datetime(tz) | ✅ | TimestampMixin |
| `updated_at` | datetime(tz)，nullable | ❌ | TimestampMixin |

约束：UNIQUE(fuel_type, effective_year) = `uq_toe_fuel_year`。
Seed：6 个 fuel_type 默认系数（`ToeConversionService.seed_defaults`，与 Alembic 迁移 2026_09_03_0800_add_toe_conversion 同步；基于蜡油加氢—综合能耗.xlsx 实例）。

7.2 新增表：htri_template_schemas（HTRI 解析模板 schema，CATEGORY_4）

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `schema_id` | UUID PK（uuid4） | ✅ | 主键 |
| `device_type` | String(20) | ✅ | ACHE / SHELL_TUBE |
| `column_count` | Integer | ✅ | 列数（ACHE 33 / SHELL_TUBE 23） |
| `columns_json` | JSONB | ✅ | [{name, type, required, unit}, ...] 列级 schema |
| `tema_type` | String(10)，nullable | ❌ | BEM/AES/AKT（仅 SHELL_TUBE 适用） |
| `version` | String(50) | ✅ | 独立版本序列 |
| `source_file_ref` | String(200)，nullable | ❌ | 原始 .xls 文件引用 |
| `created_by` | UUID，nullable | ❌ | TimestampMixin |
| `created_at` | datetime(tz) | ✅ | TimestampMixin |
| `updated_at` | datetime(tz)，nullable | ❌ | TimestampMixin |

注册入口：`TemplateService.register_htri_schema`（async instance method）。ADR-0029 决策 3：独立表，与 TemplateFile 弱耦合（不塞入 TemplateFile.columns_json）。
迁移：2026_09_03_1000_add_htri_template_schemas + alembic merge 15421f7d5afb（与 P2 Sprint 2 设备链合流）。

7.3 template_files 字段新增

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `template_version_seq` | Integer，default 0 | ✅ | V1.4 P2-OPEN-005：模板版本号字段纳入 CONFIG 模板版本序列（自增） |

配套：CATEGORY_4 DETAIL 模板资产入库（121-A-101.xls / 131-E-102-EOR.xls，seed JSON `pcs-backend/app/seeds/templates/detail_*.json`；`TemplateService` 导入时 template_version_seq 自动续号）。

7.4 关联

- ADR-0029：`docs/adr/0029-toe-conversion-and-detail-htri-templates.md`
- Task 1.10.1：ToeConversionFactor 模型 + Alembic 迁移 + API（commit 7eca3ea）
- Task 1.10.2：DETAIL 模板 seed + template_version_seq 自动续号（commit d71fe10）
- Task 1.10.3：HtriTemplateSchema + register_htri_schema（commit 18e87ef）
