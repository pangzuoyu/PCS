...
- `TODOS.md` — TODOS.md — PCS 延后工作清单 (~3843 tok)
...
- `pyproject.toml` — PCS 工艺专用综合计算软件 后端 (~287 tok)
...
- `2026_09_03_0800_add_toe_conversion.py` — add pcs_toe_conversion_factors (~688 tok)
...
- `__init__.py` — 全部 ORM 模型。alembic/env.py 依赖本包导入即注册全部表。 (~121 tok)
- `calc.py` — Declares FlashResult (~3111 tok)
- `config_domain.py` — Declares ConfigAsset (~2828 tok)
- `deliverable.py` — Declares Deliverable (~2550 tok)
- `enums.py` — Declares class (~1774 tok)
...
- `project.py` — Declares Project (~3110 tok)
...
- `category3_defaults.json` (~967 tok)
...
- `cia_engine.py` — CIA 变更影响分析引擎（Sprint 3）。 (~3492 tok)
- `coefficient_service.py` — CoefficientService — 系数库 CRUD + 批量修改（Task 2.5）。 (~1009 tok)
...
- `project_template_service.py` — ProjectTemplateService: validate_config, load (~389 tok)
- `report_service.py` — ReportService — P2 Sprint 3 Task 5.1 + 5.2。 (~1071 tok)
- `state_machine.py` — 状态机（Sprint 2 核心）。 (~3496 tok)
...
- `test_schema.py` — V3.1 Schema 层契约：表数量、关键约束、ADR-0023 复合键。 (~1493 tok)
...
- `test_category3_seeds.py` — CATEGORY_3 默认 6 张系数表 seed 测试（Task 1.8.3）。 (~750 tok)
...
- `package.json` — Node.js package manifest (~197 tok)
- `tsconfig.json` — TypeScript configuration (~146 tok)
- `vite.config.ts` (~81 tok)

## pcs-frontend/src/

- `App.tsx` — router (~173 tok)
- `main.tsx` (~101 tok)
...
- `PipingStatusPanel.tsx` — PipingStatusPanel (~297 tok)
...
- `MainLayout.tsx` — MainLayout (~373 tok)

## pcs-frontend/src/pages/

- `DashboardPage.tsx` — workspaceProjectId (~357 tok)
- `LoginPage.tsx` — MOCK_ACCOUNTS — renders form (~609 tok)
...
- `工艺专用综合计算软件需求规格说明书 Web版开发计划.md` — 安装：antd, zustand, react-router-dom, @ant-design/icons (~2971 tok)
