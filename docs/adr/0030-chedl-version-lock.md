...
选择 ChEDL 版本前，**必须**先 `dir()` 核验 P5 所需函数可用性。
...
- 若 `fluids.tanks` 函数缺失 → **预期内**（F-13-5），启用决策 7 降级预案
- 若其他模块函数缺失 → **非预期**，需重新选版本或换库；ADR 备选版本评估记录
...
4. **ADR 更新**：本 ADR 状态保持 `proposed`（长期，ChEDL 升级频繁）；决策 1/2 不变；附录记录升级历史（版本号 + 日期 + 偏差分析摘要）
...
- **RECORD_TYPE_REGISTRY 同步登记** `ChEDLVersionSnapshot`（CI baseline 类型，P5-0-6 后总注册数 = 14 类）
- **provenance 接口**：每个包装函数对应 `ChEDLProvenance` 条目（`chEDL_function` + `chEDL_version` + `known_limitations` + `fallback_available` + `fallback_formula_ref`）

## 后续

- Task 25 (P5-0-6) + Task 25b (P5-0-7) 落地本 ADR 全部 8 项决策
- Task 25 + 25b 完成后立即提交评审申请；评审委员会在 5 个工作日内决议
- **生效时机**：P5-0 批内完成（P5-1 启动前生效）
- **回退预案**：若评审未通过，包装层与版本锁定策略可分阶段保留（包装层 + dir() 核验无争议；版本锁定可回退到决策 4 的"测试以 requirements.txt 为准"过渡方案）
- **V1.8 计划同步**：Task 25 文件路径 `docs/adr/0029-chedl-version-lock.md` → `docs/adr/0030-chedl-version-lock.md`；裁决 #22 / 全局约束 / 验收段中"ADR-0029"引用 → "ADR-0030"
