...
**Parser 版本支持（三版，用户 2026-09-08 裁决）：** V2.71（1995 老格式，spec §第三部分示例同款）+ **V4.17（1998 过渡格式，sample2 未收敛参考必加）** + V8.x（2014+ 现代格式，仓库 dmc.inp + huafeng140_FCC2015.out + 200FlexiCoking1.out 同款）。dispatcher 用文件首两行 `Generated by PRO/II Keyword Generation System <version X.Y.Z>` 或输出 banner `PRO/II VERSION N.NN` 识别版本并路由到对应 lexer 子类。

- [ ] **Step 1: 写失败测试（spec §第三部分 5 边界样例）**

```python
# tests/services/test_proii_parser.py
# 注：spec §607-628 5 样例是测试构造（不是工程实例），spec §592 "PROII Lexer 5 样例 + 边界"
async def test_proii_sample1_34_components_parses():
    raw = Path("app/seeds/proii_samples/sample1_34comp.inp").read_bytes()
    result = parse_proii(raw, sample1_report)
    assert result.convergence_status == "CONVERGED"
    assert len(result.components) == 34

async def test_proii_sample2_unconverged_marks_unit_products_unreliable():
    # 参考仓库 sample/proii .out（V4.17 ELEC V12，444.9K，未收敛运行）
    # 但本测试用最小人工构造样本，不依赖仓库 sample/
    raw = Path("app/seeds/proii_samples/sample2_unconverged.inp").read_bytes()
    result = parse_proii(raw, sample2_report_with_warnings)
    assert result.convergence_status in {"NOT_CONVERGED", "ABORTED"}
    assert len(result.unreliable_unit_products) > 0

async def test_proii_sample3_side_draw_parses():
    raw = Path("app/seeds/proii_samples/sample3_side_draw.inp").read_bytes()
    result = parse_proii(raw, sample3_report)
    assert any(u.type == "SIDESTRIPPER" for u in result.units)

async def test_proii_sample4_flash_valve_parses():
    raw = Path("app/seeds/proii_samples/sample4_flash_valve.inp").read_bytes()
    result = parse_proii(raw, sample4_report)
    assert any(u.type == "FLASH" and u.has_valve for u in result.units)

async def test_proii_sample5_reactor_extraction_parses():
    raw = Path("app/seeds/proii_samples/sample5_reactor_extraction.inp").read_bytes()
    result = parse_proii(raw, sample5_report)
    assert any(u.type == "REACTOR" for u in result.units)
```

- [ ] **Step 2: RED 确认**

`cd pcs-backend && uv run pytest tests/services/test_proii_parser.py -v`
预期：FAIL（sample1~5 文件不存在 + parser 未实现）

- [ ] **Step 3: 准备 5 样例 .inp + .out（手工构造，非仓库 sample/）**

**双文件接口**（spec §1229 `parse_proii_files(sample5_inp, sample5_out)`）：每个样例 = 一对最小 .inp（输入关键字） + .out（运行报告）。

5 样例构造依据（spec §607-633，每样例覆盖一个 lexer 边界）：
- sample1_34comp：34 组分测试（spec §608），验证组分表解析
- sample2_unconverged：未收敛测试（spec §614），参考仓库 `sample/proii .out`（V4.17）的 3 WARNINGS + RUN STATISTICS 输出结构，但**手工构造最小复现 .inp + .out**（不复制仓库文件，避免版权 + 文件大小）
- sample3_side_draw：侧线抽出（spec §619），含 SIDESTRIPPER 单元
- sample4_flash_valve：闪蒸阀（spec §623），含 FLASH + VALVE
- sample5_reactor_extraction：反应萃取（spec §628 + §1229-1251 FCC 扩展），含 REACTOR + EXTRACTOR

**回归样本**（不入 git）：仓库 `sample/dmc.inp+out` / `huafeng140_FCC2015.out` / `200FlexiCoking1.out` / `proii .out` 留作 parser 鲁棒性验证（V8.x V4.17 真实工程实例），通过 `pytest -m regression` 跑（仓库现状不入 git 决策）。

**YAGNI：** 不实现 AST 完整还原（仅提取物流/单元/收敛状态/组分，不还原所有 PRO/II 关键字）。
...
7. **校验规则 §第四部分 22 条全覆盖**：本 plan SIM-7 + SIM-9 仅实现 3 类典型（相态/偏差/MW 优先级）。其余 19 条（饱和蒸汽压、临界压缩因子等）逐条覆盖需额外 ~2 天，本 Sprint 仅做骨架（TODO-035 已建）
