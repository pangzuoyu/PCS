...
def test_twu_petroleum_viscosity_available():
    from chemicals.viscosity import Twu_1985_internal  # 入参单位 °R（1.9.0 实证）
    nu = Twu_1985_internal(T=293.15 * 1.8, Tb=447.3 * 1.8, SG=0.73)  # n-癸烷，馏分式带宽
...
class PetroleumService:
    ...
    def estimate(cls, tb_c: float, sg: float) -> dict:
        ...
        # 注意：Twu_1985_internal 入参/沸点单位为 °R（Task 1.9.0 实证，vendor 源码 docstring 复现验证）
...
用已入库种子走 service 层导入（三数据源共 71 等级）：
...
- `app/seeds/pipe_classes_ppg.xlsx`（Worley PPG 压力反应器 MRQ-0001，11 等级 PROJECT：U1-U8/P1-P3）
...
注意：A2F/PSI-only 等级的 sch_series 为 `{}`，`PipeClassCreate.sch_series_json` 需允许空 dict（去掉 `min_length` 类限制或默认空 dict）——若 Step 1 的 schema 不允许，此处一并修。kaimen 种子含 design_pressure 最小 0.01（S01DC2G_R1 常压占位），schema `gt=0` 已兼容。三源 class_id 无冲突（A*/G1E vs 数字码 vs U*/P*）。
...
1. ~~P2-OPEN-001~~ **已解决（2026-09-05）**：三数据源 71 等级种子（BEP 6 COMPANY_STD + Kaimen 54 + PPG 11，均 PROJECT），Task 1.9.6 Step 5 三源导入验证
...
6. ~~三源编码是否归一~~ **已裁决（2026-09-05 用户）**：按项目绑定——每个项目从 project_pipe_classes 启用清单选等级，可自建 PROJECT 级库；编码不跨项目归一，class_id 全局唯一 PK（自建码需避免跨项目冲突，三源现状无冲突）。**遗留边界**：若未来两个项目需各自持有同码不同值的等级，须迁移为 (project_id, class_id) 复合 PK——P3 PIPE 计算接入时复核，P2 接受全局唯一约束
