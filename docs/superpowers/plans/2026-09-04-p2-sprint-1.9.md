...
**Goal:** 偏差 #4/#6/equip-lib 三项收口——PipeClass service + 5 端点、Riazi-Daubert/虚拟组分切割 service、equip-lib 沉淀 service（复用 CATEGORY_6 资产生命周期），加 Excel 批量导入满足 P2-STD-001 验收。

**Architecture:** 纯服务层 + API 增量，**零新表零迁移**（PipeClass/ProjectPipeClass ORM 已存在；equip-lib 复用 ConfigAsset CATEGORY_6 + content_json；Riazi 关联式以 CATEGORY_3 系数表 seed 入库）。物性估算为无状态纯函数 service；常数全部有可引用来源且已数值验证（见 Task 1.9.3）。
...
- Produces（P3 手动创建向导 / Task 1.9.4 seed 引用）:
  - `PetroleumService.estimate(tb_c: float, sg: float) -> dict`（键：`mw, tc_k, pc_mpa, omega, tb_k`）
...
class PetroleumService:
    ...
    def estimate(cls, tb_c: float, sg: float) -> dict:
        """馏程沸点(°C) + 比重 → MW/Tc/Pc/ω（RD80 + Kesler-Lee）。"""
        ...
        return {"tb_k": tb_c + 273.15, "mw": mw, "tc_k": tc_k, "pc_mpa": pc_mpa, "omega": omega}
...
| 项 | 理由 | 归属 |
|---|---|---|
| 粘度/Vc/Zc 关联式 | 常数无法可靠查证来源，未验证工程常数不入库；seed 已标 `out_of_scope` | P3 PETROLEUM 向导随 SUP 补 |
...
- [x] **Spec coverage**：§3.2.5 五端点 → Task 1.9.2；CRUD/项目分配/在用不可删 → 1.9.1/1.9.2；Excel 导入+模板 → 1.9.6；§3.2.6 沉淀流程/标准化校验/审批后入库 → 1.9.5；§3.2.4 V1.2 三行系数类型 → 1.9.3/1.9.4；ADR-0019 虚拟组分 25°C 切割 → 1.9.3。
- [x] **Placeholder scan**：无 TBD/TODO/“similar to”；两处标注「执行时照抄既有文件/fixture」是防错指令而非占位。
- [x] **Type consistency**：`PipeClassCreate/Update/Response`、`ProjectAssignRequest`、`EquipLibSettleRequest`、service 方法签名在 Task 间引用一致；`RD80_CONSTANTS` 在 1.9.3 与 1.9.4 数值一致。
...
1. **Subagent-Driven (recommended)** — fresh subagent per task，每 task 独立 review（spec compliance + code quality），6 task 预计 1–2 人天
