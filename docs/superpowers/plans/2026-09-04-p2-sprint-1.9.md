...
用已入库种子 `app/seeds/pipe_classes_bep_rev0.xlsx`（Worley BEP 4.3 Rev 0 提取的 6 等级：A1B/A1E/A2B/G1E/A1F/A2F）走一遍 service 层导入：
...
async def test_import_bep_seed(db):
    ...
    seed = Path(__file__).parents[2] / "app" / "seeds" / "pipe_classes_bep_rev0.xlsx"
    result = await PipeClassService.import_from_excel(db, seed.open("rb"))
...
注意：A2F 的 sch_series 为 `{}`，`PipeClassCreate.sch_series_json` 需允许空 dict（去掉 `min_length` 类限制或默认空 dict）——若 Step 1 的 schema 不允许，此处一并修。
